...
- Two buttons side by side, bottom right (stacked full-width on mobile):
  - primary "Explore Tools" → placeholder link for the future tools section
  - secondary "View Our Work" → placeholder link for the future work section (outline style using existing tokens)
- No new sections are built now. The page stays a single full-screen hero; the buttons are wired as placeholder links to be pointed at the tools and work sections once those are built.
...
- **Performance:** particle count becomes adaptive — ~20k on desktop, ~6k on tablets and mobile (based on width + `devicePixelRatio` + `hardwareConcurrency`). Pixel ratio capped at 1.5 on small screens. Respect `prefers-reduced-motion` by skipping the burst and holding a calm state.
...
- All changes are frontend only: `src/components/hero/VesperScene.tsx` and `src/routes/index.tsx`.
